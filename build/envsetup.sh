...
- tunacnremote:    Add git remote for TUNA AOSP git server.
- cloremote:       Add git remote for matching CodeLinaro repository.
- exthmremote:    Add git remote for exTHmUI Github.
...
function lineageremote()
{
    ...
    if [ -z "$REMOTE" ]
    then
        REMOTE=$(git config --get remote.exthm.projectname)
    fi
...
function tunacnremote()
{
    ...
    if [ -f ".gitupstream" ]; then
        ...
        git remote add tuna https://mirrors.tuna.tsinghua.edu.cn/git/AOSP/$PFX$PROJECT
    fi
    echo "Remote 'tuna' created"
}
...
function exthmremote()
{
    ...
    git remote add exthm https://github.com/exTHmUI/$PROJECT
